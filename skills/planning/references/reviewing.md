...
**Review early, review often** — after each non-trivial task before `Done`, before a milestone
goes to acceptance, and when stuck. Do not skip because "it's simple": simple changes are where
...
1. **Identify the change — commits are not required.** Uncommitted work is the common case; a
   commit range or a plain file list work equally well. The prompt template below covers all
   three forms. Reviewing only **inspects** the change — it never writes to version control.
2. **Dispatch a fresh reviewer** with the filled-in template. Hand it *crafted* context — the
   task file's Goal/Acceptance and the plan's Global Constraints — **never** your raw session
   history.
3. The reviewer returns findings graded **Critical / Important / Minor** plus a short assessment.

If no subagent mechanism is available at all, still review — but say plainly that the review was
inline and therefore carries the author's blind spots.

## Part 2 — Receive the findings (technical evaluation)

Read the full feedback before reacting. Then, per item: restate the requirement in your own
words, verify it against the actual codebase rather than memory, judge whether it is correct for
*this* codebase and its constraints, respond, and only then implement — one item at a time,
verifying each.
...
Fix **only** the listed findings — do not add features or unrelated changes in a fix round.
...
A review is not license to refactor unrelated code. Before finalizing fixes, walk the diff and
confirm:
...
- Anything genuinely worth doing but out of scope becomes a new task via the Phase 2
  conventions, not a change smuggled into this one.
