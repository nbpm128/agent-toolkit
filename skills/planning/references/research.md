...
2. **Frame the goal + triage** — state, in one paragraph, what success looks like, and confirm it
   with the user. Then form an **initial Confidence Score** (below). This is the triage: a
   trivial, well-understood task starts near-High and takes the **fast path** — skip the Q&A, go
   straight to step 4 with a single recommended approach, then write a short `research.md`.
